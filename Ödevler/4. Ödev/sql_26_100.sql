...
--Önce orders tablosu ile order_Details tablosu arasında bağlantı kurduk
--Sonra order_Details tablosuna product_id çekip adını aldık. İşlem önceliği
...
--47. Ortalama satış miktarımın üzerindeki satışlarım nelerdir?   ////////////BAKILACAK
...
SELECT order_id,SUM(unit_price*quantity) as amount FROM order_details
GROUP BY order_id));
--Burada subquerry yalnız bir kolon dönebildiğinden ilk karşılaşmayı kendi from'uma alıp amount değerini
--tekil olarak karşılaştırmada kullandım. aslında aynı şeyi yaptım
-------------------------------------------------------
...
--DATE_TRUNC('month', CURRENT_DATE)  -- Son ocaktan bugüne kadar olan tarihleri filtrele.Günümüz tarihine uyarlıyor.Güncel
--Burada max order_date'i kendi istediğimize göre şekillendirdik. max tarihi seç ama ayı şu gün günü şu gün olsun. yani max tarihe filtre atıyoruz.
--istersek yılı da 1997den başlatabiliyoruz.
...
--BURADA WHERE KULLANSAYDIK INNER JOINDEN SONRA, GROUP BY DAN ÖNCE KULLANACAKTIK
--HAVING KULLANARAK GRUPLADIKTAN SONRA FİLTRELEME YAPMIŞ OLDUK.RAM'DEKİ DEĞERİ GÖSTERDİK
--BİR ÖNCEKİ ÖRNEKTEKİ GİBİ GRUPLAMA YAPARKEN ÇEKECEĞİMİZ TABLOYA GÖRE İD ALDIK

--100. Hangi Müşterilerim hiç sipariş vermemiş..?

SELECT customers.customer_id FROM customers
LEFT JOIN orders ON customers.customer_id=orders.customer_id
WHERE orders.customer_id IS NULL  --burada siparişlerle eşleşen bir customer id yok ise dedik. left joini kullanma amacı buydu

--bununla ilgili başka örnekte çözelim
